...
static bool btm_prepare(struct thr_info *thr);
...
static void _init_cp2102(struct cgpu_info *bitcoinminer, uint32_t baudrate) {
        struct BTM_INFO *info = bitcoinminer->device_data;
            
        unsigned int bits = CP210X_BITS_DATA_8 | CP210X_BITS_PARITY_MARK;
        
 usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_REQUEST_IFC_ENABLE,
                                                CP210X_VALUE_UART_ENABLE, info->interface, NULL, 0, C_ENABLE_UART);
                        usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_REQUEST_DATA,
                        ...
                        usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_SET_LINE_CTL,
...
static int ch341_calculate_baudrate(unsigned int speed)
{
	const unsigned int ch341_min_rates[] = {
		CH341_MIN_RATE(0),
		CH341_MIN_RATE(1),
		CH341_MIN_RATE(2),
		CH341_MIN_RATE(3),
...
static void btm_usleep(struct BTM_INFO *info, int usec)
{
#if TUNE_CODE
...
static int btm_micro_send(struct cgpu_info *bitcoinminer, uint8_t cmd, uint8_t channel, uint8_t value)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	int bytes = 1;
	int read_bytes = 1;
...
//__
	...
	//usb_transfer(bitcoinminer, FTDI_TYPE_OUT, FTDI_REQUEST_BITMODE, usb_val, info->interface, C_SETMODEM);
	...
	if (value != 0x00 || cmd == BTM_M2_SET_VCORE) {
		bytes = 2;
	}

	usb_read_timeout(bitcoinminer, (char *)info->rx, 255, &read_bytes, 1, C_GETRESULTS);

	dumpbuffer(bitcoinminer, LOG_INFO, "(micro) TX", info->cmd, bytes);
	usb_write(bitcoinminer, (char *)info->cmd, bytes, &read_bytes, C_REQUESTRESULTS);

	memset(info->rx, 0, info->rx_len);
	usb_read_timeout(bitcoinminer, (char *)info->rx, 1, &read_bytes, 5, C_GETRESULTS);

	if (read_bytes > 0) {
		dumpbuffer(bitcoinminer, LOG_INFO, "(micro) RX", info->rx, read_bytes);
		switch (cmd) {
			case 0x20:
				temp = info->rx[0];
				...
				if (micro_temp != info->micro_temp) {
					...
					applog(LOG_WARNING, "%d: %s %d - micro temp changed to %d°C / %.1f°F",
						bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, temp, info->micro_temp);
				}
				break;
			default:
				break;
...
#define btm_send(_c, _r, _b, _crc) btm_send2(_c, _r, _b, _crc, NULL)
static void btm_send2(struct cgpu_info *bitcoinminer, unsigned char *req_tx, uint32_t bytes, uint32_t crc_bits, __maybe_unused char *msg)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	int read_bytes = 1;
...
applog(LOG_ERR, "%s()  [%02x %02x %02x %02x %02x %02x %02x %02x]", __func__,
	...
	dumpbuffer(bitcoinminer, log_level, "TX", info->cmd, bytes);
	usb_write(bitcoinminer, (char *)(info->cmd), bytes, &read_bytes, C_REQUESTRESULTS);

	//let the usb frame propagate
	if (info->asic_type == BTM_BM1397)
		btm_usleep(info, info->usb_prop);
	else
		btm_usleep(info, MS2US(1));
}

static float limit_freq(struct BTM_INFO *info, float freq, bool zero)
{
	switch(info->ident)
	{
...
static void ping_freq(struct cgpu_info *bitcoinminer, int asic)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	bool ping = false;
	...
	if (info->asic_type == BTM_BM1397)
	{
		...
		btm_send2(bitcoinminer, pingall, sizeof(pingall), 8 * sizeof(pingall) - 8, "pingfreq");
		ping = true;
	}
...
static void gsf_calc_nb2c(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	int c, i, j;
	double fac;
...
static void gc_wipe(struct BTMCHIP *gc, struct timeval *now)
{
...
static void gc_wipe_all(struct BTM_INFO *info, struct timeval *now, bool locked)
{
	int i;
...
static void gc_offset(struct BTM_INFO *info, struct BTM_ASIC_INFO *asic, struct timeval *now, bool wipe, bool locked)
{
	struct BTMCHIP *gc = &(asic->gc);
	time_t delta;
...
static void gh_offset(struct BTM_INFO *info, struct timeval *now, bool wipe, bool locked)
{
	struct BTMHASH *gh = &(info->gh);
	time_t delta;
	int i;
...
static void add_btm_nonce(struct BTM_INFO *info, struct BTM_ASIC_INFO *asic, struct timeval *now)
{
	struct BTMHASH *gh = &(info->gh);
...
static double btm_gh_hashrate(struct BTM_INFO *info, struct timeval *now, bool locked)
{
	struct BTMHASH *gh = &(info->gh);
...
static void job_offset(struct BTM_INFO *info, struct timeval *now, bool wipe, bool locked)
{
	struct BTMJOB *job = &(info->job);
	time_t delta;
	int jobnow;
...
static void add_btm_job(struct BTM_INFO *info, struct timeval *now, bool wipe)
{
	struct BTMJOB *job = &(info->job);
	bool firstjob;
	double avg;
...
// limit to max diff of 16 unless the chips x cores is a bit better than a BTV1
...
static void set_ticket(struct cgpu_info *bitcoinminer, float diff, bool force, bool locked)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	struct timeval now;
	bool got = false;
...
//		return;
	...
	btm_send2(bitcoinminer, ticket, sizeof(ticket), 8 * sizeof(ticket) - 8, "ticket");
	if (!force)
		btm_usleep(info, MS2US(10));
	else
		btm_usleep(info, MS2US(20));

	applog(LOG_ERR, "%d: %s %d - set ticket to 0x%02x/%u work %u/%.1f",
		bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id,
...
static double noncepercent(struct BTM_INFO *info, int chip, struct timeval *now)
{
...
static void calc_gsf_freq(struct cgpu_info *bitcoinminer, float frequency, int chip)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	char chipn[8];
	bool doall;
	...
	else
	{
		if (chip < 0 || chip >= (int)(info->chips))
		{
			applog(LOG_ERR, "%d: %s %d - invalid set chip [%d] -> freq %.2fMHz",
				bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, chip, frequency);
			return;
		}
		doall = false;
	}
	...
	if (doall)
	{
		...
		{
			btm_usleep(info, MS2US(10));
			btm_send2(bitcoinminer, prefreqall, sizeof(prefreqall), 8 * sizeof(prefreqall) - 8, "prefreq");
		}
		for (i = 0; i < 2; i++)
		{
			btm_usleep(info, MS2US(10));
			btm_send2(bitcoinminer, freqbufall, sizeof(freqbufall), 8 * sizeof(freqbufall) - 8, "freq");
		}
	...
	else
	{
		...
		{
			btm_usleep(info, MS2US(10));
			btm_send2(bitcoinminer, prefreqch, sizeof(prefreqch), 8 * sizeof(prefreqch) - 8, "prefreq");
		}
		for (i = 0; i < 2; i++)
		{
			btm_usleep(info, MS2US(10));
			btm_send2(bitcoinminer, freqbufch, sizeof(freqbufch), 8 * sizeof(freqbufch) - 8, "freq");
		}
...
//	applog(LOG_INFO, "%d: %s %d - setting frequency to %.2fMHz (%.2f)" " (%.0f/%.0f/%.0f/%.0f)",
	applog(LOG_ERR, "%d: %s %d - setting [%s] frequency to %.2fMHz (%.2f)" " (%.0f/%.0f/%.0f/%.0f)",
		bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, chipn, frequency, newf, fa, fb, fc1, fc2);

	ping_freq(bitcoinminer, 0);
}

static void btm_send_chain_inactive(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	unsigned int i, j;

	applog(LOG_INFO,"%d: %s %d - sending chain inactive for %d chip(s)",
		bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->chips);
	...
	if (info->asic_type == BTM_BM1397)
	{
		...
		for (i = 0; i < 3; i++)
		{
			btm_send2(bitcoinminer, chainin, sizeof(chainin), 8 * sizeof(chainin) - 8, "chin");
			btm_usleep(info, MS2US(100));
		}
		...
		for (i = 0; i < info->chips; i++)
		{
			...
			btm_send2(bitcoinminer, chippy, sizeof(chippy), 8 * sizeof(chippy) - 8, "chippy");
			btm_usleep(info, MS2US(10));
		}
		...
		btm_send2(bitcoinminer, init1, sizeof(init1), 8 * sizeof(init1) - 8, "init1");
		btm_usleep(info, MS2US(10));
		btm_send2(bitcoinminer, init2, sizeof(init2), 8 * sizeof(init2) - 8, "init2");
		btm_usleep(info, MS2US(100));
		btm_send2(bitcoinminer, init3, sizeof(init3), 8 * sizeof(init3) - 8, "init3");
		btm_usleep(info, MS2US(50));
		btm_send2(bitcoinminer, init4, sizeof(init4), 8 * sizeof(init4) - 8, "init4");
		btm_usleep(info, MS2US(100));

		// set ticket based on chips, pool will be above this anyway
		set_ticket(bitcoinminer, 0.0, true, false);
		...
		for (j = 0; j < 2; j++)
		{
			btm_send2(bitcoinminer, init5, sizeof(init5), 8 * sizeof(init5) - 8, "init5");
			btm_usleep(info, MS2US(50));
		}
		btm_send2(bitcoinminer, init6, sizeof(init6), 8 * sizeof(init6) - 8, "init6");
		btm_usleep(info, MS2US(100));
        unsigned char baudrate[] = { 0x51, 0x09, 0x00, 0x18, 0x00, 0x00, 0x61, 0x31, 0x00 }; // lo 1M



		//unsigned char baudrate[] = { 0x51, 0x09, 0x00, 0x18, 0x00, 0x00, 0x64, 0x31, 0x00 }; // lo 1.51M
		//unsigned char baudrate[] = { 0x51, 0x09, 0x00, 0x18, 0x00, 0x00, 0x65, 0x31, 0x00 }; // lo 500000k
		//unsigned char baudrate[] = { 0x51, 0x09, 0x00, 0x18, 0x00, 0x00, 0x73, 0x31, 0x00 }; // lo 1.51M
		info->bauddiv = 1; // 1.5M

        if(info->ident != IDENT_BTV1) {
		baudrate[5] = 0x00;
		baudrate[6] = 0x7A;
		}
		// 3.125M/27
		...
		if (info->midstates == 4)
		{
			...
			// 3.125M/27
			//info->bauddiv = 26;
			//baud_val = ch341_calculate_baudrate(115200);
			//baud_rate = 115200
		}
#endif

		applog(LOG_ERR, "%d: %s %d - setting bauddiv : ",
			bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
		btm_send2(bitcoinminer, baudrate, sizeof(baudrate), 8 * sizeof(baudrate) - 8, "baud");
		btm_usleep(info, MS2US(10));
    ...
    if(info->ident == IDENT_BTV1) {
		uint32_t baudrate2 = 1500000;
		usb_transfer_data(bitcoinminer,CP210X_TYPE_OUT, CP210X_REQUEST_BAUD,0,info->interface,&baudrate2,sizeof(baudrate2),C_SETBAUD);
	} else if(info->ident == IDENT_POCKET) {
	    int baud_val = ch341_calculate_baudrate(115200);
		//libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x1312, baud_val, NULL, 0, 1000);
	
		//libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x0f2c, 0, NULL, 0, 1000);
        libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), CH341_REQ_WRITE_REG, (uint32_t)CH341_REG_DIVISOR << 8 | CH341_REG_PRESCALER, baud_val, 0, 0, 1000);

//    libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x1312, 0xcc03, NULL, 0, 1000);
	//libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x0f2c, 0x0008, NULL, 0, 1000);
	}

		btm_usleep(info, MS2US(10));

		calc_gsf_freq(bitcoinminer, info->frequency, -1);

		btm_usleep(info, MS2US(20));
	} 

	if (info->mining_state == BTM_MINER_CHIP_COUNT_OK) {
		applog(LOG_INFO, "%d: %s %d - open cores",
			bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
		info->zero_check = 0;
		info->task_hcn = 0;
		info->mining_state = BTM_MINER_OPEN_CORE;
	}
}

static void btm_update_rates(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	struct BTM_ASIC_INFO *asic;
	...
	if (average_frequency != info->frequency) {
		applog(LOG_INFO,"%d: %s %d - frequency updated %.2fMHz -> %.2fMHz",
			bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->frequency, average_frequency);
	...
	if (info->asic_type != BTM_BM1397)
	{
		info->ticket_mask = btm_bound(pow(2, ceil(log(info->hashrate / (2.0 * 0xffffffffull)) / log(2))) - 1, 0, 4000);
		info->ticket_mask = info->ticket_mask;
		info->difficulty = info->ticket_mask + 1;
	}

	info->wait_factor = info->wait_factor0;
	if (!opt_nerdaxe_noboost && info->vmask && (info->asic_type == BTM_BM1397))
		info->wait_factor *= info->midstates;
	est = info->wait_factor * (float)(info->fullscan_us);
	info->max_task_wait = btm_bound((uint64_t)est, 1, 3 * info->fullscan_us);

	if (info->asic_type == BTM_BM1397)
	{
		// 90% will always allow at least 2 freq steps
		if (opt_nerdaxe_tune_up > 90)
			opt_nerdaxe_tune_up = 90;
		else
			info->tune_up = opt_nerdaxe_tune_up;
	}
	else
		info->tune_up = 99;
	...
	else
	{
		...
		if (info->asic_type != BTM_BM1397 && info->chips > 1)
	...
	if(info->ident == IDENT_BTV1) {
	    ...
	    if((now.tv_sec - info->last_send_hashrate_display.tv_sec) >= 60) {
	           ...
	           uint16_t c = 0x0002;
	            ...
	            int read_bytes = 0;
	            char empty[] = "0\r\n";
	            usb_write(bitcoinminer, &empty[0], strlen(empty), &read_bytes, C_REQUESTRESULTS);
	            if(bitcoinminer->hash_rate > 0) {
			      //  applog(LOG_ERR,"Hashrate: %lu",bitcoinminer->hash_rate / 1000000000);
			        char send[64];
			        memset(&send[0],0,64);
			        int len = snprintf(&send[0],64,"hash_se:%lu\r\n",bitcoinminer->hash_rate / 1000000000);
			       // applog(LOG_ERR,"Send %s Len:%d\n",send,len);
			        if(len > 0) {			        
	                    usb_write(bitcoinminer, &send[0], len, &read_bytes, C_REQUESTRESULTS);
			        }
			
	                btm_usleep(info, MS2US(2));
			    } else {
			        char send[64] = "hash_se:0\r\n";
			        
			       // applog(LOG_ERR,"Send %s\n",send);
			        
	                usb_write(bitcoinminer, &send[0], strlen(send), &read_bytes, C_REQUESTRESULTS);
	                btm_usleep(info, MS2US(2));
...
//	c |= 0x0200;
	        ...
	        usb_write(bitcoinminer, &dummy[0], sizeof(dummy), &read_bytes, C_REQUESTRESULTS);
	        ...
	        btm_usleep(info, MS2US(1));
	        */
	        cgtime(&info->last_send_hashrate_display);    
	    }
	}
...
static void btm_set_frequency_single(struct cgpu_info *bitcoinminer, float frequency, int asic_id)
{
	//struct BT_INFO *info = bitcoinminer->device_data;
...
static void btm_set_frequency(struct cgpu_info *bitcoinminer, float frequency)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	...
	if (info->asic_type == BTM_BM1397) {
		calc_gsf_freq(bitcoinminer, frequency, -1);
	} 
	btm_update_rates(bitcoinminer);
...
static void btm_update_work(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	int i;
...
static void btm_flush_buffer(struct cgpu_info *bitcoinminer)
{
	int read_bytes = 1;
	unsigned char resp[32];

	while (read_bytes) {
		usb_read_timeout(bitcoinminer, (char *)resp, 32, &read_bytes, 1, C_REQUESTRESULTS);
	}
}

static void btm_flush_work(struct cgpu_info *bitcoinminer)
{
	btm_flush_buffer(bitcoinminer);
	btm_update_work(bitcoinminer);
}
/*
static void _init_cp2102(struct cgpu_info *bitcoinminer, uint32_t baudrate) {
        struct BTM_INFO *info = bitcoinminer->device_data;
...
*/
static void btm_toggle_reset(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	unsigned short usb_val;
...
uint32_t read_bytes = 0;
	...
	usb_write(bitcoinminer, &test1[0], 3, &read_bytes, C_REQUESTRESULTS);
	char test2[] = "core_en:1\r\n";
	
	
	usb_write(bitcoinminer, &test2[0], 11, &read_bytes, C_REQUESTRESULTS);
	
	char test3[] = "core_rs:1\r\n";
	
	usb_write(bitcoinminer, &test3[0], 11, &read_bytes, C_REQUESTRESULTS);
	
	btm_usleep(info, MS2US(100));
	
	
	char test4[] = "core_rs:0\r\n";
	
	usb_write(bitcoinminer, &test4[0], 11, &read_bytes, C_REQUESTRESULTS);
	...
	int r = libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0xa1, 0, 0, NULL, 0, 1000);
    if (r < 0) {
        applog(LOG_ERR, "failed control transfer 0xa1\n");
    }
    r = libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x2518, CH341_LCR_ENABLE_RX | CH341_LCR_ENABLE_TX | CH341_LCR_CS8, NULL, 0, 1000);
    if (r < 0) {
        applog(LOG_ERR, "failed control transfer 0x9a,0x2518\n");
    }
    
    r = libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0xa1, 0x501f, 0xd90a, NULL, 0, 1000);
    if (r < 0) {
        applog(LOG_ERR, "failed control transfer 0xa1,0x501f\n");
    }



	if(libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x1312, 0xcc03, NULL, 0, 1000) < 0) {
		applog(LOG_ERR,"Error set baudrate");
	}
	if(libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0x9a, 0x0f2c, 0x0008, NULL, 0, 1000) <0 ) {
		applog(LOG_ERR,"Error set baudrate control register");
	}


	libusb_control_transfer(bitcoinminer->usbdev->handle, (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_ENDPOINT_OUT), 0xa4, ~(1 << 6), 0, NULL, 0, 1000);

	btm_usleep(info, MS2US(500));
...
/*
	...
	usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_REQUEST_IFC_ENABLE,
			...
			usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_SET_LINE_CTL,
						bits, info->interface, NULL, 0, C_SETPARITY);*/
	//int baud_val = ch341_get_divisor(115200);
   // usb_transfer(bitcoinminer, CH341_REQ_WRITE_REG, CH341_REG_DIVISOR << 8 | CH341_REG_PRESCALER, baud_val,0 , C_SETBAUD);
 
 } else {
		applog(LOG_ERR,"Device not found");
    abort();
 
 }
	btm_usleep(info, MS2US(30));

	if (info->asic_type == BTM_BM1397)
		btm_usleep(info, MS2US(1000));
	else
		btm_usleep(info, MS2US(30));



	btm_usleep(info, MS2US(200));

	cgtime(&info->last_reset);
}

static void btm_gsf_nonce(struct cgpu_info *bitcoinminer, K_ITEM *item)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	unsigned char *rx = BTM_DATA_NONCE(item)->rx;
	int hwe = bitcoinminer->hw_errors;
	...
	if (asic_id >= (int)(info->chips))
        {
		applog(LOG_INFO, "%d: %s %d - nonce %08x @ %02x rx[4] %02x invalid asic_id (0..%d)",
			bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, nonce, job_id,
...
else
{
	...
	struct BTM_ASIC_INFO *asic = &info->asics[asic_id];
	...
	if (nonce == asic->prev_nonce)
	{
		...
		if (info->dups == 1)
			info->mining_state = BTM_MINER_MINING_DUPS;
		mutex_unlock(&info->lock);
	...
	applog(LOG_INFO, "%d: %s %d - Device reported nonce: %08x @ %02x (%d)",
		bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, nonce, job_id, info->tracker);

	if (!opt_nerdaxe_noboost && info->vmask)
	{
	...
	if (!ok)
	{
		...
		for (i = 0; !ok && i < (int)BTM_CUR_ATTEMPT; i++)
		{
			w_job_id = BTM_JOB_ID_ROLL(info->job_id, BTM_cur_attempt[i], info) & 0xfc;
			work = info->work[w_job_id];
			active_work = info->active_work[w_job_id];
			if (work && active_work)
			{
				for (midnum = 0; !ok && midnum < domid; midnum++)
				{
					...
					if ((diff = test_nonce_value(work, nonce)) != 0)
					{
						...
						applog(LOG_INFO, "%d: %s %d - Nonce Recovered : %08x @ job[%02x]->fix[%02x] len %u prelen %u",
							bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id,
							nonce, job_id, w_job_id, (uint32_t)(BTM_DATA_NONCE(item)->len),
							(uint32_t)(BTM_DATA_NONCE(item)->prelen));
					}
				}
			}
		}
		...
		if (!ok)
		{
			applog(LOG_INFO, "ERR: %d: %s %d - Nonce Dumped : %08x @ job[%02x] cur[%02x] diff %u",
				bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id,
				...
				if (diff < ticket_1397[i].hi_limit)
				{
					...
					if (info->ticket_failures > MAX_TICKET_CHECK)
					{
						...
						applog(LOG_ERR, "%d: %s %d - ticket %u failed too many times setting to max",
							bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, ticket_1397[i].diff);
						//set_ticket(compac, 1.0, true, true);
						set_ticket(bitcoinminer, 0.0, true, true);
						info->ticket_ok = true;
						break;
					}
	...
	if (active_work && work && submit_nonce(info->thr, work, nonce))
	{
		...
		if (midnum > 0)
		{
			applog(LOG_INFO, "%d: %s %d - AsicBoost nonce found : midstate %d",
				bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, midnum);
		}
		...
		if (info->nb2c_setup)
			add_btm_nonce(info, asic, &(BTM_DATA_NONCE(item)->when));
		else
			add_btm_nonce(info, NULL, &(BTM_DATA_NONCE(item)->when));
	}
	else
	{
		// shouldn't be possible since diff has already been checked
		if (hwe != bitcoinminer->hw_errors)
		{
			mutex_lock(&info->lock);
			cgtime(&info->last_hwerror);
			mutex_unlock(&info->lock);
...
static uint64_t btm_check_nonce(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	uint32_t nonce = 0;
	int hwe = bitcoinminer->hw_errors;
...
static void busy_work(struct BTM_INFO *info)
{
	...
	if (info->asic_type == BTM_BM1397) {
		...
		info->task[3] = ((!opt_nerdaxe_noboost && info->vmask) ? 0x04 : 0x01);
...
static void init_task(struct BTM_INFO *info)
{
	...
	if (info->asic_type == BTM_BM1397) {
		...
		info->task[1] = info->task_len;
		info->task[2] = info->job_id & 0xff;
		info->task[3] = ((!opt_nerdaxe_noboost && info->vmask) ? info->midstates : 0x01);

		if (info->mining_state == BTM_MINER_MINING) {
			btm_stuff_reverse(info->task + 8, work->data + 64, 12);
			btm_stuff_reverse(info->task + 20, work->midstate, 32);
			if (!opt_nerdaxe_noboost && info->vmask) {
				if (info->midstates > 1)
					btm_stuff_reverse(info->task + 20 + 32, work->midstate1, 32);
				if (info->midstates > 2)
					btm_stuff_reverse(info->task + 20 + 32 + 32, work->midstate2, 32);
				if (info->midstates > 3)
					btm_stuff_reverse(info->task + 20 + 32 + 32 + 32, work->midstate3, 32);
			}
		} else {
			memset(info->task + 8, 0xff, 12);
		}
...
static void change_freq_any(struct cgpu_info *bitcoinminer, float new_freq)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	unsigned int i;
	float old_freq;
	...
	for (i = 0; i < info->chips; i++)
	{
		struct BTM_ASIC_INFO *asic = &info->asics[i];
		...
		{
			if (i == 0)
				btm_set_frequency(bitcoinminer, new_freq);
		}
...
static void *btm_mine2(void *object)
{
	struct cgpu_info *bitcoinminer = (struct cgpu_info *)object;
	struct BTM_INFO *info = bitcoinminer->device_data;
	struct work *work = NULL;
	...
	applog(LOG_INFO, "%d: %s %d - work thread niceness (%d)",
		bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, ret_nice);

	sleep_us = 100;

	cgtime(&last_plateau_check);

	while (info->mining_state != BTM_MINER_SHUTDOWN)
	{
		if (old_work)
		{
			mutex_lock(&info->lock);
			work_completed(bitcoinminer, old_work);
			mutex_unlock(&info->lock);
			old_work = NULL;
		}

		if (info->chips == 0
		||  bitcoinminer->deven == DEV_DISABLED
		||  bitcoinminer->usbinfo.nodev
		||  (info->mining_state != BTM_MINER_MINING && info->mining_state != BTM_MINER_MINING_DUPS))
		{
			btm_usleep(info, MS2US(10));
			continue;
		}
		...
		sleep_us = btm_bound(ceil(info->max_task_wait / 20), 1, 100 * 1000); // 1us .. 100ms

		dev_runtime = cgpu_runtime(bitcoinminer);
		wu = bitcoinminer->diff1 / dev_runtime * 60;
		...
		if (ms_tdiff(&now, &info->last_reset) > MS_SECOND_10)
		{
			if (ms_tdiff(&now, &last_rolling) >= MS_SECOND_1)
			{
				...
				{
					cgtime(&last_rolling);
					info->rolling = btm_gh_hashrate(info, &last_rolling, true);
				}
				mutex_unlock(&info->ghlock);
			}
			...
			&&  frequency_computed <= info->frequency)
			{
				...
				applog(LOG_INFO, "%d: %s %d - new comp=%.2f (gs=%.2f)",
					bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, frequency_computed,
			...
			if (ms_tdiff(&now, &last_plateau_check) > MS_SECOND_5)
			{
				...
				for (i = 0; i < info->chips; i++)
				{
					...
					if (info->asic_type == BTM_BM1397)
					{
						...
						{
							plateau_type = BTM_PT_NONONCE;
							applog(LOG_ERR, "%d: %s %d - plateau_type PT_NONONCE [%u] %d > %.2f (lock=%d)",
								bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, i,
					...
					{
						plateau_type = BTM_PT_FREQSET;
						applog(LOG_INFO, "%d: %s %d - plateau_type PT_FREQSET [%u] %u > 3",
							bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, i, asic->frequency_attempt);
					}
					...
					if (plateau_type)
					{
						...
						{
							struct BTM_ASIC_INFO *asjc = &info->asics[j];
							sprintf(freq_chip_buf, "[%d:%.2f]", j, asjc->frequency);
							strcat(freq_buf, freq_chip_buf);
						}
						applog(LOG_INFO,"%d: %s %d - %s",
							bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, freq_buf);
						...
						{
						 case BTM_PT_FREQNR:
							applog(LOG_ERR,"%d: %s %d - no frequency reply from chip[%u] - %.2fMHz",
								bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, i, info->frequency);
							asic->frequency_attempt = 0;
							reason = " FREQNR";
							break;
						 case BTM_PT_FREQSET:
							applog(LOG_ERR,"%d: %s %d - frequency set fail to chip[%u] - %.2fMHz",
								bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, i, info->frequency);
							asic->frequency_attempt = 0;
							reason = " FREQSET";
							break;
						 case BTM_PT_NONONCE:
							applog(LOG_ERR,"%d: %s %d - missing nonces from chip[%u] - %.2fMHz",
								bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, i, info->frequency);
							reason = " NONONCE";
							break;
						 default:
						...
						if (plateau_type == BTM_PT_NONONCE || info->asic_type == BTM_BM1397)
						{
							// BM1384 is less tolerant to sudden drops in frequency.
							// Ignore other indicators e
							cc++;
							doreset = true;
						}
						else
						...
						if (doreset)
						{
							...
							if(cc == 4) {
							   ...
							   applog(LOG_ERR,"Do reset");
							   new_frequency = opt_nerdaxe_freq;
							   info->frequency_requested = opt_nerdaxe_freq;
							   info->frequency_fail_high = opt_nerdaxe_freq;
							   info->frequency_start = opt_nerdaxe_start_freq;
				                btm_toggle_reset(bitcoinminer);
				                info->mining_state = BTM_MINER_CHIP_COUNT_OK;
				                
							} else {
							    info->mining_state = BTM_MINER_RESET;
						    }
						}
						//applog(LOG_ERR,"F: %f",info->frequency_fail_high);
			...
			&&  ms_tdiff(&now, &info->last_reset) < info->ramp_time)
			{
				...
				&&  (info->gh.noncesum > GHNONCENEEDED))
				{
					...
					frequency_updated = 1;
					change_freq_any(bitcoinminer, new_frequency);
			...
			else
			{
				...
				&&  (ms_tdiff(&now, &info->tune_limit) >= MS_MINUTE_2))
				{
					...
					mutex_lock(&info->ghlock);
					curr_hr = btm_gh_hashrate(info, &now, true);
					...
					if ((last == (GHNUM-1)) || (nonces > GHNONCES))
					{
						...
						/*if (curr_hr < (hash_for_freq * info->ghrequire))
						{
							...
							mutex_lock(&info->lock);
							frequency_updated = 1;
							change_freq_any(bitcoinminer, info->frequency_start);
						...
						if (opt_nerdaxe_tune2 != 0
						&&  (info->frequency_requested < info->frequency_selected)
						&&  (tdiff(&now, &info->last_reset) >= ((double)opt_nerdaxe_tune2 * 60.0))
						&&  (tdiff(&now, &info->last_tune_up) >= ((double)opt_nerdaxe_tune2 * 60.0)))
						{
							...
							if (new_freq <= info->frequency_selected)
							{
								...
								frequency_updated = 1;
								change_freq_any(bitcoinminer, new_freq);
							}
...
applog(LOG_ERR, "%s() %d: %s %d - Task [%02x] len %3u", __func__,
	bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, jid, task_len);
...
applog(LOG_ERR, " [%02x %02x %02x %02x %02x %02x %02x %02x]",
	info->task[24], info->task[25], info->task[26], info->task[27], info->task[28], info->task[29], info->task[30], info->task[31]);
		...
		err = usb_write(bitcoinminer, (char *)info->task, task_len, &sent_bytes, C_SENDWORK);
		//dumpbuffer(compac, LOG_WARNING, "TASK.TX", info->task, task_len);
		if (err != LIBUSB_SUCCESS)
		{
			applog(LOG_WARNING,"%d: %s %d - usb failure (%d)", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, err);
			info->mining_state = BTM_MINER_RESET;
			continue;
		}
		if (sent_bytes != task_len)
		{
			if (ms_tdiff(&now, &info->last_write_error) > (5 * 1000)) {
				applog(LOG_WARNING,"%d: %s %d - usb write error [%d:%d]",
					bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, sent_bytes, task_len);
		...
		else
		{
			...
			add_btm_job(info, &now, false);
			job_added = true;
		}

		//let the usb frame propagate
		if (info->asic_type == BTM_BM1397 && info->usb_prop != 1000)
			btm_usleep(info, info->usb_prop);
		else
			btm_usleep(info, MS2US(1));
...
static void *btm_handle_rx(void *object, int read_bytes, int path)
{
	struct cgpu_info *bitcoinminer = (struct cgpu_info *)object;
	struct BTM_INFO *info = bitcoinminer->device_data;
	struct BTM_ASIC_INFO *asic;
	...
	if (path) {
		dumpbuffer(bitcoinminer, log_level, "RX1", info->rx, read_bytes);
	} else {
		dumpbuffer(bitcoinminer, log_level, "RX0", info->rx, read_bytes);
	}
			...
			if (frequency_of != (int)(info->chips)) {
				...
				if (frequency != asic->frequency) {
					...
					if (frequency < asic->frequency && frequency != info->frequency_requested) {
						applog(LOG_INFO,"%d: %s %d - chip[%d] reported frequency at %.2fMHz", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->frequency_of, frequency);
					} else {
						applog(LOG_INFO,"%d: %s %d - chip[%d] reported new frequency of %.2fMHz", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->frequency_of, frequency);
					}
					...
					if (info->frequency_syncd != syncd) {
						...
						applog(LOG_INFO,"%d: %s %d - syncd [%d]", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, syncd);
					}
				} else {
					applog(LOG_INFO,"%d: %s %d - chip[%d] reported frequency of %.2fMHz", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->frequency_of, frequency);
				}
			} else {
	...
	switch (info->mining_state) {
		case BTM_MINER_CHIP_COUNT:
		case BTM_MINER_CHIP_COUNT_XX:
			if (cmd_resp && (info->rx[0] == 0x13 ||
			    (info->rx[0] == 0xaa && info->rx[1] == 0x55 && info->rx[2] == 0x13))) { // BM1397
				struct BTM_ASIC_INFO *asic = &info->asics[info->chips];
				memset(asic, 0, sizeof(struct BTM_ASIC_INFO));
				...
				cgtime(&asic->last_nonce);
				info->chips++;
				info->mining_state = BTM_MINER_CHIP_COUNT_XX;
				btm_update_rates(bitcoinminer);
			}
			break;
		case BTM_MINER_OPEN_CORE:
			if ((info->rx[0] == 0x72 && info->rx[1] == 0x03 && info->rx[2] == 0xEA && info->rx[3] == 0x83) ||
				(info->rx[0] == 0xE1 && info->rx[1] == 0x6B && info->rx[2] == 0xF8 && info->rx[3] == 0x09)) {
...
#endif
				mutex_lock(&info->lock);
				info->hashes += btm_check_nonce(bitcoinminer);
				mutex_unlock(&info->lock);
			}
			break;
		default:
...
static void *btm_gsf_nonce_que(void *object)
{
	struct cgpu_info *bitcoinminer = (struct cgpu_info *)object;
	struct BTM_INFO *info = bitcoinminer->device_data;
	...
	while (info->mining_state != BTM_MINER_SHUTDOWN)
	{
		...
		if (item)
		{
			btm_gsf_nonce(bitcoinminer, item);
			K_WLOCK(info->nlist);
...
static bool gsf_reply(struct BTM_INFO *info, unsigned char *rx, int len, struct timeval *now)
{
	...
	if (len == (int)(info->rx_len) && rx[7] == BM1397FREQ)
	{
		...
		if (chip >= 0 && chip < (int)(info->chips))
		{
			struct BTM_ASIC_INFO *asic = &info->asics[chip];
...
static void *btm_listen2(struct cgpu_info *bitcoinminer, struct BTM_INFO *info)
{
	...
	while (info->mining_state != BTM_MINER_SHUTDOWN)
	{
		...
		if (info->mining_state == BTM_MINER_CHIP_COUNT)
		{
		...
		usb_read_timeout(bitcoinminer, ((char *)rx)+pos, BUFFER_MAX-pos, &read_bytes, tmo, C_GETRESULTS);
		pos += read_bytes;
			...
			switch (info->mining_state)
			{
			 ...
			 case BTM_MINER_CHIP_COUNT_XX:
				...
				// ignore all data until we get at least 1 chip reply
			 	if (!chipped && info->mining_state == BTM_MINER_CHIP_COUNT_XX)
				{
...
applog(LOG_ERR, " [%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x]",
	rx[0], rx[1], rx[2], rx[3], rx[4], rx[5], rx[6], rx[7], rx[8], rx[9], rx[10], rx[11]);
				...
				if (!used)
				{
					...
					K_WUNLOCK(info->nlist);
					BTM_DATA_NONCE(item)->asic = 0;
					// should never be true ...
					if (len > (int)sizeof(BTM_DATA_NONCE(item)->rx))
						len = (int)sizeof(BTM_DATA_NONCE(item)->rx);
					memcpy(BTM_DATA_NONCE(item)->rx, rx, len);
					BTM_DATA_NONCE(item)->len = len;
					BTM_DATA_NONCE(item)->prelen = prelen;
					BTM_DATA_NONCE(item)->when.tv_sec = now.tv_sec;
					BTM_DATA_NONCE(item)->when.tv_usec = now.tv_usec;
					K_WLOCK(info->nlist);
...
applog(LOG_ERR, " [%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x]",
	rx[0], rx[1], rx[2], rx[3], rx[4], rx[5], rx[6], rx[7], rx[8], rx[9], rx[10], rx[11]);
		...
		{
			if (info->mining_state == BTM_MINER_CHIP_COUNT_XX)
			{
...
static void *btm_listen(void *object)
{
	struct cgpu_info *bitcoinminer = (struct cgpu_info *)object;
	struct BTM_INFO *info = bitcoinminer->device_data;

	if (info->asic_type == BTM_BM1397)
		return btm_listen2(bitcoinminer, info);
	...
	while (info->mining_state != BTM_MINER_SHUTDOWN) {
		...
		if (info->mining_state == BTM_MINER_CHIP_COUNT) {
			...
			usb_read_timeout(bitcoinminer, (char *)rx, BUFFER_MAX, &read_bytes, 1000, C_GETRESULTS);
			dumpbuffer(bitcoinminer, LOG_INFO, "CMD.RX", rx, read_bytes);
			...
			if (rx_bytes >= (BUFFER_MAX - info->rx_len)) {
				applog(LOG_INFO, "%d: %s %d - Buffer not useful, dumping (b) %d bytes", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, rx_bytes);
				dumpbuffer(bitcoinminer, LOG_INFO, "NO-CRC-RX", rx, rx_bytes);
				pos = 0;
				rx_bytes = 0;
			}

			usb_read_timeout(bitcoinminer, (char *)(&rx[pos]), info->rx_len, &read_bytes, 20, C_GETRESULTS);
			rx_bytes += read_bytes;
			pos = rx_bytes;
		}
		...
		if (read_bytes > 0) {
			if (rx_bytes < info->rx_len) {
				applog(LOG_INFO, "%d: %s %d - Buffered %d bytes", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, rx_bytes);
				dumpbuffer(bitcoinminer, LOG_INFO, "Partial-RX", rx, rx_bytes);
				continue;
			}
			...
			if (info->mining_state == BTM_MINER_CHIP_COUNT_XX || cmd_resp) {
				if (rx_bytes % info->rx_len == 2) {
					...
					rx_bytes -= shift;
					pos = rx_bytes;
					applog(LOG_INFO, "%d: %s %d - Extra Data - 0x01 0x%02x", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, extra & 0xff);
				}
			}
			...
			if (rx_bytes >= info->rx_len) {
				...
				for (i = 0; i <= (rx_bytes - info->rx_len); i++) {
					...
					if (rx_okay) {
						...
						btm_handle_rx(bitcoinminer, info->rx_len, 0);
...
*/
			...
			if (rx_bytes > 0) {
				applog(LOG_INFO, "%d: %s %d - Second read, no data dumping (c) %d bytes", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, rx_bytes);
				dumpbuffer(bitcoinminer, LOG_INFO, "EXTRA-RX", rx, rx_bytes);
			}
			...
			switch (info->mining_state) {
				case BTM_MINER_CHIP_COUNT_XX:
					if (info->chips < info->expected_chips) {
						applog(LOG_INFO, "%d: %s %d - found %d/%d chip(s)", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->chips, info->expected_chips);
						info->mining_state = BTM_MINER_RESET;
					} else {
						applog(LOG_INFO, "%d: %s %d - found %d chip(s)", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id, info->chips);
						if (info->chips > 0) {
							info->mining_state = BTM_MINER_CHIP_COUNT_OK;
							mutex_lock(&static_lock);
							(*init_count) = 0;
...
static bool btm_init(struct thr_info *thr)
{
	int i;
	struct cgpu_info *bitcoinminer = thr->cgpu;
	struct BTM_INFO *info = bitcoinminer->device_data;
	float step_freq;
	...
	if (opt_nerdaxe_tune_down > 100)
		opt_nerdaxe_tune_down = 100;
	if (opt_nerdaxe_tune_up > 99)
		opt_nerdaxe_tune_up = 99;
	opt_nerdaxe_wait_factor = fbound(opt_nerdaxe_wait_factor, 0.01, 2.0);
	info->wait_factor0 = opt_nerdaxe_wait_factor;
	if (opt_nerdaxe_start_freq < 25)
		opt_nerdaxe_start_freq = 25;
	if (opt_nerdaxe_step_freq < 1)
		opt_nerdaxe_step_freq = 1;
	if (opt_nerdaxe_step_delay < 1)
		opt_nerdaxe_step_delay = 1;

	// must limit it to allow tune downs before trying again
	if (opt_nerdaxe_tune2 < 30 && opt_nerdaxe_tune2 != 0)
		opt_nerdaxe_tune2 = 30;
	...
	info->step_freq = FREQ_BASE(opt_nerdaxe_step_freq);
	...
	switch (info->ident)
	{
	 case IDENT_BTV1:
		if (info->ident == IDENT_BTV1)
			info->frequency_requested = limit_freq(info, opt_nerdaxe_freq, false);

		info->frequency_start = limit_freq(info, opt_nerdaxe_start_freq, false);
		...
		step_freq = opt_nerdaxe_step_freq;
		if (step_freq == 6.25)
			step_freq = 5.0;
	...
	if (!info->rthr.pth) {
		...
		if (info->ident == IDENT_BTV1)
		{
			info->nlist = k_new_list("BTMinerNonces", sizeof(struct BTM_NONCE),
		...
		if (thr_info_create(&(info->rthr), NULL, btm_listen, (void *)bitcoinminer)) {
			applog(LOG_ERR, "%d: %s %d - read thread create failed", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
			return false;
		} else {
			applog(LOG_INFO, "%d: %s %d - read thread created", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
		}
		pthread_detach(info->rthr.pth);

		btm_usleep(info, MS2US(100));

		if (thr_info_create(&(info->wthr), NULL, btm_mine2, (void *)bitcoinminer)) {
			applog(LOG_ERR, "%d: %s %d - write thread create failed", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
			return false;
		} else {
			applog(LOG_INFO, "%d: %s %d - write thread created", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
		}

		pthread_detach(info->wthr.pth);

		if (info->ident == IDENT_BTV1 || info->ident == IDENT_POCKET)
		{
			btm_usleep(info, MS2US(10));
			...
			if (thr_info_create(&(info->nthr), NULL, btm_gsf_nonce_que, (void *)bitcoinminer))
			{
...
static int64_t btm_scanwork(struct thr_info *thr)
{
	struct cgpu_info *bitcoinminer = thr->cgpu;
	struct BTM_INFO *info = bitcoinminer->device_data;
	...
	switch (info->mining_state) {
		case BTM_MINER_INIT:
			...
			info->mining_state = BTM_MINER_CHIP_COUNT;
			return 0;
			break;
		case BTM_MINER_CHIP_COUNT:
			if (ms_tdiff(&now, &info->last_reset) > MS_SECOND_5) {
				applog(LOG_INFO, "%d: %s %d - found 0 chip(s)", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
				info->mining_state = BTM_MINER_RESET;
				return 0;
			}
			btm_usleep(info, MS2US(10));
			break;
		case BTM_MINER_CHIP_COUNT_OK:
			btm_usleep(info, MS2US(50));
			btm_set_frequency(bitcoinminer, info->frequency_start);
			btm_send_chain_inactive(bitcoinminer);

			if (info->asic_type == BTM_BM1397)
				info->mining_state = BTM_MINER_OPEN_CORE_OK;
		...
		case BTM_MINER_OPEN_CORE:
			...
			init_task(info);
			dumpbuffer(bitcoinminer, LOG_DEBUG, "RAMP", info->task, info->task_len);

			usb_write(bitcoinminer, (char *)info->task, info->task_len, &read_bytes, C_SENDWORK);
			...
			return 0;
			break;
		case BTM_MINER_OPEN_CORE_OK:
			applog(LOG_INFO, "%d: %s %d - start work", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
			if (info->asic_type == BTM_BM1397)
				gsf_calc_nb2c(bitcoinminer);
			cgtime(&info->start_time);
			cgtime(&info->monitor_time);
			cgtime(&info->last_frequency_adjust);
			...
			btm_update_rates(bitcoinminer);
			info->update_work = 1;
			info->mining_state = BTM_MINER_MINING;
			return 0;
			break;
		case BTM_MINER_MINING:
			break;
		case BTM_MINER_RESET:
			btm_flush_work(bitcoinminer);
			if (info->asic_type == BTM_BM1397) {
				btm_toggle_reset(bitcoinminer);
			} 
			btm_prepare(thr);
			...
			info->fail_count++;
			info->dupsreset = 0;
			info->mining_state = BTM_MINER_INIT;
			cgtime(&info->last_reset);
...
static struct cgpu_info *btm_detect_one(struct libusb_device *dev, struct usb_find_devices *found)
{
	struct cgpu_info *bitcoinminer;
	struct BTM_INFO *info;
	int i;
	bool exclude_me = 0;
	uint32_t baudrate = CP210X_DATA_BAUD;
	unsigned int bits = CP210X_BITS_DATA_8 | CP210X_BITS_PARITY_MARK;

	bitcoinminer = usb_alloc_cgpu(&bitcoinminer_drv, 1);
	applog(LOG_INFO, "Try detect bitcoinminer");

	if (!usb_init(bitcoinminer, dev, found))
	{
		applog(LOG_INFO, "failed usb_init");
		bitcoinminer = usb_free_cgpu(bitcoinminer);
		return NULL;
	}

	// all zero
	info = cgcalloc(1, sizeof(struct BTM_INFO));

#if TUNE_CODE
	pthread_mutex_init(&info->slock, NULL);
#endif

	bitcoinminer->device_data = (void *)info;

	info->ident = usb_ident(bitcoinminer);
	...
	switch (info->ident)
	{
        ...
        usb_transfer_data(bitcoinminer, CP210X_TYPE_OUT, CP210X_REQUEST_IFC_ENABLE,
			...
			break;
		default:
			quit(1, "%d: %s btm_detect_one() invalid %s ident=%d",
				bitcoinminer->cgminer_id, bitcoinminer->drv->dname, bitcoinminer->drv->dname, info->ident);
	}
	...
	info->interface = usb_interface(bitcoinminer);
	info->mining_state = BTM_MINER_INIT;

	applog(LOG_DEBUG, "Using interface %d", info->interface);

	if (!add_cgpu(bitcoinminer))
		quit(1, "Failed to add_cgpu in compac_detect_one");

	update_usb_stats(bitcoinminer);

	if(bitcoinminer->unique_id == NULL) {

		bitcoinminer->unique_id = malloc(9);
		for(int i=0;i<8;i++) {
			bitcoinminer->unique_id[i] = '0' + (rand() / (RAND_MAX / (9 + 1) + 1));
		}
		bitcoinminer->unique_id[8] = 0;
	}
	//for (i = 0; i < 8; i++)
	//	bitcoinminer->unique_id[i] = bitcoinminer->unique_id[i+3];
	//bitcoinminer->unique_id[8] = 0;
...
static void btm_detect(bool __maybe_unused hotplug)
{

	applog(LOG_DEBUG, "btm_detect");
	usb_detect(&bitcoinminer_drv, btm_detect_one);
}
...
static bool btm_prepare(struct thr_info *thr)
{
	struct cgpu_info *bitcoinminer = thr->cgpu;
	struct BTM_INFO *info = bitcoinminer->device_data;
	int device = (bitcoinminer->usbinfo.bus_number * 0xff + bitcoinminer->usbinfo.device_address) % 0xffff;
	...
	if (info->init_count == 1) {
		applog(LOG_WARNING, "%d: %s %d - %s (%s)",
			bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id,
			bitcoinminer->usbdev->prod_string, bitcoinminer->unique_id);
	} else {
	...
	if (info->init_count != 0 && info->init_count % 5 == 0) {
		applog(LOG_INFO, "%d: %s %d - forcing usb_nodev()", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
		usb_nodev(bitcoinminer);
...
static void btm_statline(char *buf, size_t bufsiz, struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	struct timeval now;
	unsigned int i;
	...
	if (opt_widescreen) {
		...
		for (i = 1; i <= info->chips; i++) {
			struct BTM_ASIC_INFO *asic = &info->asics[i - 1];

			switch (asic->state) {
				case BTM_ASIC_HEALTHY:
					asic_stat[i] = 'o';
					break;
				case BTM_ASIC_HALFDEAD:
					asic_stat[i] = '-';
					break;
				case BTM_ASIC_ALMOST_DEAD:
					asic_stat[i] = '!';
					break;
				case BTM_ASIC_DEAD:
					asic_stat[i] = 'x';
...
static struct api_data *btm_api_stats(struct cgpu_info *bitcoinminer)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	...
	root = api_add_string(root, "Serial", bitcoinminer->usbdev->serial_string, false);
	...
	mutex_lock(&info->ghlock);
	ghs = btm_gh_hashrate(info, &now, true) / 1.0e3;
	...
	for (i = 0; i < (int)BTM_CUR_ATTEMPT; i++)
	{
		snprintf(nambuf, sizeof(nambuf), "cur_off_%d_%d", i, BTM_cur_attempt[i]);
...
static void btm_shutdown(struct thr_info *thr)
{
	struct cgpu_info *bitcoinminer = thr->cgpu;
	struct BTM_INFO *info = bitcoinminer->device_data;
	applog(LOG_INFO, "%d: %s %d - shutting down", bitcoinminer->cgminer_id, bitcoinminer->drv->name, bitcoinminer->device_id);
	if (!bitcoinminer->usbinfo.nodev) {
		if (info->asic_type == BTM_BM1397) {
			calc_gsf_freq(bitcoinminer, 0, -1);
			btm_toggle_reset(bitcoinminer);
		}
	}
	info->mining_state = BTM_MINER_SHUTDOWN;
...
uint64_t btm_bound(uint64_t value, uint64_t lower_bound, uint64_t upper_bound)
{
	if (value < lower_bound)
		return lower_bound;
	if (value > upper_bound)
...
void btm_stuff_reverse(unsigned char *dst, unsigned char *src, uint32_t len)
{
	uint32_t i;
	for (i = 0; i < len; i++) {
		dst[i] = src[len - i - 1];
...
static char *btm_api_set(struct cgpu_info *bitcoinminer, char *option, char *setting, char *replybuf, size_t siz)
{
	struct BTM_INFO *info = bitcoinminer->device_data;
	float freq;
	...
	if (strcasecmp(option, "freq") == 0)
	{
		...
		change_freq_any(bitcoinminer, freq);
	...
	if (strcasecmp(option, "chip") == 0)
	{
		...
		calc_gsf_freq(bitcoinminer, freq, chip);
	...
	if (strcasecmp(option, "usbprop") == 0)
	{
		...
		info->usb_prop = (int)btm_bound(atoi(setting), USLEEPMIN, 1000);
...
struct device_drv bitcoinminer_drv = {
	.drv_id              = DRIVER_bitcoinminer,
	.dname               = "BitcoinMiner",
	.name                = "BTMDriver",
	.hash_work           = hash_queued_work,
	.get_api_stats       = btm_api_stats,
	.get_statline_before = btm_statline,
	.set_device	     	 = btm_api_set,
	.drv_detect          = btm_detect,
	.scanwork            = btm_scanwork,
	.flush_work          = btm_flush_work,
	.update_work         = btm_update_work,
	.thread_prepare      = btm_prepare,
	.thread_init         = btm_init,
	.thread_shutdown     = btm_shutdown,
};
