...
enum btm_miner_state {
	BTM_MINER_INIT = 1,
	BTM_MINER_CHIP_COUNT,	// 2
	BTM_MINER_CHIP_COUNT_XX,	// 3
	BTM_MINER_CHIP_COUNT_OK,	// 4
	BTM_MINER_OPEN_CORE,	// 5
	BTM_MINER_OPEN_CORE_OK,	// 6
	BTM_MINER_MINING,		// 7
	BTM_MINER_MINING_DUPS,	// 8
	BTM_MINER_SHUTDOWN,	// 9
	BTM_MINER_SHUTDOWN_OK,	// 10
	BTM_MINER_RESET		// 11
};
...
enum btm_micro_command {
	BTM_M1_GET_FAN    = (0x00 << 3),
	BTM_M1_GET_RPM    = (0x01 << 3),
	BTM_M1_GET_VIN    = (0x02 << 3),
	BTM_M1_GET_IIN    = (0x03 << 3),
	BTM_M1_GET_TEMP   = (0x04 << 3),
	BTM_M1_GET_VNODE0 = (0x05 << 3),

	BTM_M1_CLR_BEN    = (0x08 << 3),
	BTM_M1_SET_BEN    = (0x09 << 3),
	BTM_M1_CLR_LED    = (0x0A << 3),
	BTM_M1_SET_LED    = (0x0B << 3),
	BTM_M1_CLR_RST    = (0x0C << 3),
	BTM_M1_SET_RST    = (0x0D << 3),

	BTM_M2_SET_FAN    = (0x18 << 3),
	BTM_M2_SET_VCORE  = (0x1C << 3)
};

enum btm_asic_state {
	BTM_ASIC_HEALTHY = 0,
	BTM_ASIC_HALFDEAD,
	BTM_ASIC_ALMOST_DEAD,
	BTM_ASIC_DEAD
};
...
struct BTM_ASIC_INFO {
	...
	enum btm_asic_state state;
	enum btm_asic_state last_state;
	...
	int nonces;
	struct BTMCHIP gc;	// running nonce buffer
};
...
#define BTM_DATA_NONCE(_item) ((struct BTM_NONCE *)(_item->data))
#define ALLOC_NLIST_ITEMS 256
#define LIMIT_NLIST_ITEMS 0

// BM1397 info->job_id offsets to check (when job_id is wrong)
static int BTM_cur_attempt[] = { 0, -4, -8, -12 };
#define BTM_CUR_ATTEMPT (sizeof(BTM_cur_attempt)/sizeof(int))
...
#define BTM_JOB_ID_ROLL(_jid, _add, _info) \
...
// a time jump without any nonces will reset the BTHASH data
...
struct BTM_INFO {
	...
	enum btm_miner_state mining_state;	// Miner state
	enum btm_miner_asic asic_type;	// ASIC Type
	...
	struct timeval last_send_hashrate_display;       // Last time we send hashrate to display

	struct timeval first_task;
	uint64_t tasks;
	uint64_t cur_off[BTM_CUR_ATTEMPT];
	...
	struct BTM_ASIC_INFO asics[255];
...
void btm_stuff_lsb(unsigned char *dst, uint32_t x);
void btm_stuff_msb(unsigned char *dst, uint32_t x);
