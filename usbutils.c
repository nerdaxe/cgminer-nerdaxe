...
static struct usb_epinfo btm_epinfos_pocket[] = {
...
static struct usb_intinfo btm_ints_pocket[] = {
	USB_EPS(0, btm_epinfos_pocket)
};
...
*/
	...
	for (ifinfo = 0; ifinfo < found->intinfo_count; ifinfo++) {
		...
		applog(LOG_DEBUG, "device T1: %d  packet size %d addr %d",epdesc->bmAttributes,epdesc->wMaxPacketSize,epdesc->bEndpointAddress);
		applog(LOG_DEBUG, "T2: att %d size %d ep %d",epinfos[epinfo].att,epinfos[epinfo].size,epinfos[epinfo].ep);
